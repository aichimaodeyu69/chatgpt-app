...
export default function ModelSelect() {
    ...
    return (
        <div className="flex bg-gray-100 dark:bg-gray-900 p-1 rounded">
            {models.map((item) => {
                const selected = item.name===currentModel
                return <button key={item.id} className={`group hover:text-gray-900 hover:dark:text-gray-100 flex justify-center items-center space-x-2 py-2.5 min-w-[148px] text-sm font-medium border rounded-lg ${
                    selected 
