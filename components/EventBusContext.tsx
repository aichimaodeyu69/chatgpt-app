...
import { ReactNode, createContext, useContext, useMemo, useCallback, useState } from "react"
...
export default function EventBusContextProvider({
    ...
    const unsubscribe = useCallback((event: string, callback: EventListener) => {
        if (listeners[event]) {
            listeners[event] = listeners[event].filter((cb) => {
                cb !== callback
            })
        }
        setListeners({ ...listeners })
    ...
    return <EventBusContext.Provider value={contextValue}>
        {children}
